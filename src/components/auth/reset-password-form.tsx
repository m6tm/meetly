...
export default function ResetPasswordForm() {
	...
	useEffect(() => {
		if (!token && !successMessage) {
			// Only set error if not already in a success state
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (!token && !successMessage) {
			// Prevent submission if token is missing and not already succeeded
			...
			<form onSubmit={handleSubmit}>
				<CardContent className="grid gap-4">
					...
					<Button
						...
						type="submit"
						disabled={
							isLoading || !!successMessage || (!token && !successMessage)
						}
					>
