		...
		const notifications = response.data.map((notif) => {
			...
			switch (notif.type) {
				case "MEETING_REMINDER":
					icon = (
						<Mail className="h-4 w-4 text-purple-500 group-focus:text-accent-foreground" />
					);
				...
				case "NEWS_UPDATE":
					icon = (
						<MessageSquare className="h-4 w-4 text-blue-500 group-focus:text-accent-foreground" />
					);
					type = "News Update";
					break;
				...
				case "SECURITY_ALERT":
					icon = (
						<Settings className="h-4 w-4 text-red-500 group-focus:text-accent-foreground" />
					);
					type = "Security Alert";
					break;
				...
				case "TEAM_ACTIVITY":
					icon = (
						<UserCircle className="h-4 w-4 text-orange-500 group-focus:text-accent-foreground" />
					);
					type = "Team Activity";
					break;
				...
				case "TRANSCRIPTION_UPDATE":
					icon = (
						<CheckCircle className="h-4 w-4 text-green-500 group-focus:text-accent-foreground" />
					);
	...
	const columns: ColumnDef<NotificationItem>[] = React.useMemo(
		() => [
			...
			},
			{
				...
				header: "Content",
				cell: ({ row }) => (
					<div className="truncate max-w-md" title={row.original.content}>
			...
			},
			{
				...
				cell: ({ row }) => (
					<Badge
						variant={row.original.read ? "secondary" : "default"}
						className={row.original.read ? "font-normal" : "bg-primary/80"}
					>
			...
			<Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
				<DialogContent className="max-w-5xl max-h-[90vh] flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>Manage your notifications</DialogDescription>
					</DialogHeader>
