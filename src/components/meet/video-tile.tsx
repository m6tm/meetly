...
import { Track, type VideoTrack as VideoTrackT } from "livekit-client";
...
const VideoTile: React.FC<VideoTileProps> = ({
	...
	const showVideo =
		!isVideoOff && (isUser ? hasCameraPermission === true : true);
	const participant = isLocal
		? useLocalParticipant().localParticipant
		: useRemoteParticipant(participantId)!;

	// Adjust sizes based on whether it's the main screen or a smaller tile
	const avatarSizeClass = isMainScreen
		? "h-24 w-24 sm:h-32 sm:w-32"
		: "h-12 w-12 sm:h-16 sm:w-16";
	const avatarFallbackSizeClass = isMainScreen
		? "text-4xl sm:text-5xl"
		: "text-xl sm:text-2xl";
	const nameTextSizeClass = isMainScreen
		? "text-base sm:text-lg"
		: "text-[10px] sm:text-xs";
	const statusIconSizeClass = isMainScreen
		? "h-4 w-4 sm:h-5 sm:w-5"
		: "h-3 w-3 sm:h-4 sm:w-4";
	const nameBarPaddingClass = isMainScreen
	...
	return (
		<div className="bg-gray-800 rounded-lg flex items-center justify-center relative overflow-hidden w-full h-full group">
			...
			>
				{isMuted ? (
					<MicOff
						className={cn(
							statusIconSizeClass,
							"mr-1 text-red-400 flex-shrink-0",
						)}
					/>
				) : (
					<Mic className={cn(statusIconSizeClass, "mr-1 flex-shrink-0")} />
				)}
			...
			{participant && getParticipantHandUp(participant) && (
				<span
					className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-blue-600/80 p-1 sm:p-1.5 rounded-full z-10"
					title="Main levée"
				>
			...
			{onToggleFeature && (
				...
				>
					<Button
						...
						title={
							isCurrentlyFeatured
								? "Détacher le participant"
								: "Mettre en avant le participant"
						}
					>
						{isCurrentlyFeatured ? (
							<PinOff className={statusIconSizeClass} />
						) : (
							<Pin className={statusIconSizeClass} />
						)}
					</Button>
				</div>
			)}
