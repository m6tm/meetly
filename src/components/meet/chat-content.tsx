...
import type { MeetlyReceivedChatMessage } from "./types";
...
const ChatContent: React.FC<ChatContentProps> = ({
	...
	const pinnedMessagesInChat = messages.filter((msg) =>
	...
	const handlePrevPinned = () => {
		setCurrentPinnedIndex(
			(prev) =>
				(prev - 1 + pinnedMessagesInChat.length) % pinnedMessagesInChat.length,
		);
	};
	...
	return (
		<>
			{displayedPinnedMessage && (
				<div className="p-2 sm:p-3 border-b border-gray-700 flex-shrink-0 bg-gray-700/50">
					<div
						...
						onClick={() =>
							scrollToAndHighlightMessage(displayedPinnedMessage.id)
						}
					...
					>
						<div className="flex-grow min-w-0">
							<div className="text-xs text-blue-300 mb-0.5 sm:mb-1 flex items-center">
								...
								Écrit par{" "}
								{displayedPinnedMessage.from &&
									getParticipantName(displayedPinnedMessage.from)}
							</div>
					...
					{pinnedMessagesInChat.length > 1 && (
						<div className="flex items-center justify-center mt-1.5 sm:mt-2 space-x-1 sm:space-x-2">
							<Button
								...
								className="text-gray-300 hover:text-white h-6 w-6 sm:h-7 sm:w-7"
							>
			...
			>
				...
				{messages.map((msg, index) => {
					...
					const showSenderName =
						(!msg.from || !msg.from.isLocal) &&
						(!prevMessage ||
							(prevMessage &&
								prevMessage.from &&
								msg.from &&
								getParticipantName(prevMessage.from) !==
									getParticipantName(msg.from)) ||
							(prevMessage.from && prevMessage.from.isLocal));
					...
					return (
						...
						>
							...
							>
								...
								<p className="whitespace-pre-wrap break-words flex-grow text-xs sm:text-sm">
									{msg.message.text}
								</p>
								<Button
									variant="ghost"
								...
								>
									{isCurrentlyPinned ? (
										<PinOff className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
									) : (
							...
							<span
								className={cn(
									"text-xs text-gray-500 mt-0.5 sm:mt-1 px-1.5 sm:px-2",
									msg.message.isSelf ? "text-right w-full" : "text-left w-full",
								)}
							>
			...
			<form
				...
				className="p-2 sm:p-3 border-t border-gray-700 flex-shrink-0"
			>
				<div className="relative flex items-center">
					...
					<Button
						...
						variant="ghost"
						size="icon"
						className="absolute right-1 sm:right-1.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white h-7 w-7 sm:h-8 sm:w-8"
