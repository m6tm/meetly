...
import {
	...
	getParticipantHandUp,
	getParticipantJoined,
	getParticipantMetadata,
	getParticipantName,
	getParticipantRole,
	setParticimantMetadata,
...
import ParticipantsContent, {
	type LiveKitMessage,
} from "./participants-content";
import SidePanelContainer from "./side-panel-container";
import type { MeetlyReceivedChatMessage, Message, Participant } from "./types";
...
const MeetingLayout: React.FC<MeetingLayoutProps> = ({
	...
	const [featuredParticipantId, setFeaturedParticipantId] = useState<
		string | null
	>(null);
	...
	const handleDataReceived = useCallback(
		async (payload: Uint8Array, participant: RemoteParticipant | undefined) => {
			try {
				...
				switch (message.type) {
					case "ROLE_CHANGE":
						if (message.data.participantId && message.data.role) {
							...
							if (
								localParticipant &&
								message.data.participantId === localParticipant.identity
							) {
		...
		[localParticipant],
	); // Dependencies will be adjusted later
	...
	const handleShareScreen = async () => {
		try {
			...
			if (
				...
				message =
					"Screen sharing is not allowed by your browser or system settings. Please check the permissions policy.";
	...
	const hasCameraPermission =
		cameraTrack !== undefined || cameraTrack === null
			? true
			: cameraTrack === undefined
				? null
				: false;

	const mappedRemoteParticipants: Participant[] = remoteParticipants
		.filter((participant) => {
	...
	const allParticipantsForLayout = [
		userParticipant,
		...mappedRemoteParticipants,
	];
	...
	if (featuredParticipantId) {
		const featuredP = allParticipantsForLayout.find(
			(p) => p.id === featuredParticipantId,
		);
		const otherParticipants = allParticipantsForLayout.filter(
			(p) => p.id !== featuredParticipantId,
		);
		...
		return (
			<div className="flex h-screen w-screen bg-gray-900 text-white relative overflow-hidden select-none">
				<div className={cn(mainContentClasses, "flex-col md:flex-row")}>
					{featuredP &&
						featuredP.participant && ( // Add check for featuredP.participant
						...
						<ScrollArea className="flex-grow w-full md:w-auto md:max-w-[200px] lg:max-w-[240px] h-2/5 md:h-full p-1 min-h-0 md:border-l border-gray-700/50">
							<div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-1 gap-1 sm:gap-2">
								{otherParticipants.map((participant) => (
									<div
										key={participant.id}
										className="aspect-video bg-gray-800 rounded-lg relative overflow-hidden shadow-md"
									>
										<VideoTile
											key={participant.id}
											...
											screenShareTrack={
												...
												participant.isScreenSharing
													...
													: remoteParticipants
															.find((p) => p.identity === participant.id)
															?.getTrackPublication(Track.Source.ScreenShare)
															?.videoTrack
											} // Pass screen share track
								...
								{otherParticipants.length === 0 && featuredP && (
									<div className="text-center text-gray-500 text-xs p-4 h-full flex items-center justify-center">
										No other <br />
										participants.
									</div>
								)}
