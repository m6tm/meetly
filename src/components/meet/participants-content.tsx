...
import { UserPlus, Search, ChevronUp, ChevronDown, MoreVertical, Loader2, ShieldCheck, UserX, ShieldClose } from 'lucide-react';
...
import { banneParticipant, changeParticipantRole, inviteParticipantToMeet } from '@/actions/meetly-meet-manager';
...
const ParticipantsContent: React.FC<ParticipantsContentProps> = ({
  ...
  const handleSetRole = async (participant: Participant, role: ParticipantRole) => {
  ...
  const handleSendInvite = async () => {
    ...
    if (!response.success) {
      toast({
        ...
        description: response.error || "Impossible d'envoyer l'invitation. Veuillez réessayer.",
        variant: "destructive",
      });
      setIsSendingInvite(false);
    }
