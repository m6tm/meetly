...
import { ConnectionState, type LocalVideoTrack, Track } from "livekit-client"; // Import Track type
import {
	Loader2,
	Mic,
	MicOff,
...
const LobbyView: React.FC<LobbyViewProps> = ({
	...
	const {
		activeDeviceId: selectedMicId,
		setActiveMediaDevice: setSelectedMicById,
	} = useMediaDeviceSelect({ kind: "audioinput" });

	const cameraDevices = useMediaDevices({ kind: "videoinput" });
	const {
		activeDeviceId: selectedCamId,
		setActiveMediaDevice: setSelectedCamById,
	} = useMediaDeviceSelect({ kind: "videoinput" });

	const speakerDevices = useMediaDevices({ kind: "audiooutput" });
	const {
		activeDeviceId: selectedSpeakerId,
		setActiveMediaDevice: setSelectedSpeakerById,
	...
	const hasCameraPermission =
		cameraTrack !== undefined || cameraTrack === null
			...
			: cameraTrack === undefined
				? null
				: false; // Simplified permission check based on track availability
			...
			<div className="flex flex-col lg:flex-row w-full max-w-6xl h-full max-h-[700px] lg:max-h-[calc(100vh-4rem)] gap-4 sm:gap-6 md:gap-8">
				<div className="flex-grow lg:w-2/3 flex flex-col h-1/2 lg:h-full">
					<div className="relative aspect-video w-full bg-muted rounded-xl overflow-hidden shadow-2xl flex items-center justify-center border flex-grow min-h-0">
						{hasCameraPermission === false ? (
							...
							<div className="flex flex-col items-center text-muted-foreground">
								...
								{cameraTrack === undefined && (
									<p className="mt-2 sm:mt-3 text-sm sm:text-md">
					...
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mt-3 sm:mt-4">
						<Select
							value={selectedMicId}
							onValueChange={setSelectedMicById}
							disabled={hasCameraPermission === false}
						>
							{" "}
							{/* Use hook state and handler */}
						...
						<Select
							...
							onValueChange={setSelectedSpeakerById}
							disabled={hasCameraPermission === false}
						>
							{" "}
							{/* Use hook state and handler */}
						...
						<Select
							value={selectedCamId}
							onValueChange={setSelectedCamById}
							disabled={hasCameraPermission === false}
						>
							{" "}
							{/* Use hook state and handler */}
				...
				<div className="lg:w-1/3 flex flex-col justify-center items-center lg:items-start space-y-4 sm:space-y-5 p-4 sm:p-0">
					<h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-foreground text-center lg:text-left">
						Prêt à participer ?
					</h2>
					<p className="text-xs sm:text-sm text-muted-foreground text-center lg:text-left">
						Personne d'autre ne participe à cet appel.
					</p>

					<div className="w-full space-y-1.5 sm:space-y-2">
						<Label
							htmlFor="displayNameLobby"
							className="text-xs sm:text-sm font-medium text-muted-foreground"
						>
							Votre nom pour la réunion
						</Label>
						<Input
							id="displayNameLobby"
