...
import { type LocalParticipant, RoomEvent } from "livekit-client";
import {
	CircleDot,
	Clock,
	Hand,
			...
			<div className="flex items-center justify-between max-w-4xl mx-auto">
				...
				<div className="hidden md:flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm text-gray-300">
					<span className="flex items-center">
						<Clock className="h-3 w-3 sm:h-4 sm:w-4 mr-1" /> {currentTimeState}
					</span>
					...
					<Button
						...
						className={cn(
							...
							participant.isScreenShareEnabled &&
								"bg-blue-600 hover:bg-blue-700", // Highlight when sharing
						)}
						aria-label={
							participant.isScreenShareEnabled
					...
					{_participant.role !== "participant" && (
						<Button
							...
							className={cn(
								"text-white hover:bg-gray-700/70 p-2 sm:p-2.5 rounded-full h-9 w-9 sm:h-10 sm:w-10",
								isRecording &&
									"bg-gray-700/70 hover:bg-gray-700/90 animate-pulse",
							)}
							aria-label={
								isRecording
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button
								variant="ghost"
								size="icon"
								className="text-white hover:bg-gray-700/70 p-2 sm:p-2.5 rounded-full h-9 w-9 sm:h-10 sm:w-10"
							>
						...
						>
							...
							>
								...
								<Badge
									variant="default"
									className="ml-auto bg-blue-500 text-white text-[10px] h-4 w-4 min-w-4 rounded-full flex items-center justify-center p-0"
								>
									{currentParticipantsCount}
								</Badge>
							</DropdownMenuItem>
				...
				<div className="hidden md:flex items-center space-x-1 sm:space-x-2">
					...
					>
						...
						<Badge
							variant="default"
							className="absolute -top-0.5 -right-0.5 bg-blue-500 text-white text-[9px] sm:text-[10px] h-3.5 w-3.5 sm:h-4 sm:w-4 min-w-[0.875rem] sm:min-w-4 rounded-full flex items-center justify-center p-0"
						>
							{currentParticipantsCount}
						</Badge>
					</Button>
