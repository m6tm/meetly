...
import type { Meeting } from "@/app/dashboard/meetings/page";
...
export default function ScheduleMeetingModal({
	...
	const [isDatePopoverOpen, setIsDatePopoverOpen] = React.useState(
		!!initialValues,
	);
	...
	const handleSchedule = async () => {
		...
		const response = initialValues
			? await updateMeet(data as UpdateMeetType)
		...
		if (!response.success) {
			toast({
				...
				description:
					response.error || "An error occurred while scheduling the meeting.",
				variant: "destructive",
			});
		...
		<Dialog open={isDialogOpen} onOpenChange={handleOpenDialog}>
			...
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Fill in the details below to schedule your new meeting. Click save
						when you&apos;re done.
					</DialogDescription>
				...
				<div className="grid gap-4 py-4">
					...
					<div className="grid grid-cols-4 items-center gap-4">
						<Label
							htmlFor="recurring"
							className="text-right col-start-1 col-span-1 flex items-center justify-end"
						>
