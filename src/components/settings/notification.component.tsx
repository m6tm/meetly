...
} from "@/actions/settings-notifications.actions";
...
export default function NotificationComponent() {
	...
	const handleGetPreferences = useCallback(async () => {
		...
		form.reset({
			meetingReminder:
				response.data.find((p) => p.type === NotificationType.MEETING_REMINDER)
					?.enabled || false,
			transcriptionUpdate:
				response.data.find(
					(p) => p.type === NotificationType.TRANSCRIPTION_UPDATE,
				)?.enabled || false,
			teamActivity:
				response.data.find((p) => p.type === NotificationType.TEAM_ACTIVITY)
					?.enabled || false,
			newsletter:
				response.data.find((p) => p.type === NotificationType.NEWS_UPDATE)
					?.enabled || false,
		});
		setIsLoading(false);
	}, []);
			...
			<Card className="shadow-md">
				...
				<CardContent className="space-y-6">
					...
					<div className="flex items-center space-x-2">
						...
						/>
						<Label
							htmlFor="email-meeting-reminders"
					...
					<div className="flex items-center space-x-2">
						...
						/>
						<Label
							htmlFor="email-transcription-updates"
							className="font-normal cursor-pointer"
						>
					...
					<div className="space-y-1">
						...
						<p className="text-xs text-muted-foreground">
							Get real-time alerts on your devices (requires app installation or
							browser permission).
						</p>
					</div>
					<div className="flex items-center space-x-2">
						...
						>
							Enable All Push Notifications (Coming Soon)
						</Label>
					</div>
				</CardContent>
