...
import type {
	...
	ParticipantRole,
	RecordingStartData,
	StopRecordingPayload as TStopRecordingPayload,
} from "@/types/meetly.types";
import { verifyPassword } from "@/utils/secure";
import { createClient } from "@/utils/supabase/server";
import {
	createMeetTokenValidator,
	startMeetRecorderValidator,
	stopMeetRecorderValidator,
...
export async function generateMeetTokenAction(
	data: MeetTokenDataType,
): Promise<ActionResponse<GenerationMeetTokenResponse>> {
	...
	const isAdmin =
		!isModerator &&
		meeting &&
		meeting.invitees.some(
			(invite) => invite.email === user.email && invite.role === "admin",
		);
	...
	try {
		...
		const invite = meeting.invitees.find(
...
export async function banneParticipant(
	participant_id: string,
	meet_code: string,
): Promise<ActionResponse> {
	const supabase = await createClient();
...
export async function inviteParticipantToMeet(
	email: string,
	meet_code: string,
): Promise<ActionResponse> {
	const supabase = await createClient();
	...
	if (meeting.invitees.some((invite) => invite.email.trim() === email))
		return {
			success: true,
			error: null,
			data: null,
	...
	await prisma.meetingInvitation.create({
		data: {
			email: inviteUser ? (inviteUser.email ?? email.trim()) : email.trim(),
	...
	try {
		...
		if (list && list.length > 0) {
			...
			const stopRecordingPayload: TStopRecordingPayload = {
				...
				meetingId: meeting.id,
				retry_count: 0,
				duration: egressInfo.fileResults
					.map((file) => file.duration.toString())
					.at(0),
			};
...
export async function getRecordingSignedUrl(
	...
	try {
		...
		const bucket =
			process.env.NEXT_PUBLIC_SUPABASE_STORAGE_MEETINGS_BUCKET || "";
