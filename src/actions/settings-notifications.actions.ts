...
import type { NotificationForm } from "@/components/settings/notification.component";
import { getPrisma } from "@/lib/prisma";
import type { ActionResponse } from "@/types/action-response";
import { createClient } from "@/utils/supabase/server";
import { updateNotificationValidator } from "@/validators/settings.validators";

export const getNotificationPreferencesAction = async (): Promise<
...
export const updateNotificationPreferencesAction = async (
	...
	const updatePromises = Object.entries(typeMapping).map(
		([formField, notificationType]) => {
