	...
	async createBucket(
		...
		try {
			...
			if (error) {
				return {
					success: false,
					error: `Erreur création bucket: ${error.message}`,
				};
			}
...
async function exempleUtilisation() {
	...
	const largeFileResult = await uploader.uploadLargeFileWithS3(
		largeFileBuffer,
		{
			bucket: "my-bucket",
	...
	const signedUrlResult = await uploader.createSignedUrl(
		"my-bucket",
		"documents/test.txt",
		3600,
	);
