...
export default function SubscriptionPage() {
	...
	const handleCancelSubscription = async () => {
		...
		toast({
			...
			description:
				"Your subscription cancellation is being processed. We're sorry to see you go!",
			variant: "destructive",
		});
	...
	const handleSelectPlan = async (planId: string, planName: string) => {
		if (planId === "enterprise") {
			toast({
				...
				description:
					"We will get in touch with you shortly regarding the Enterprise plan.",
			});
			return;
		}
	...
	const confirmDeletePaymentMethod = async () => {
		...
		setSavedPaymentMethods((prev) =>
			prev.filter((pm) => pm.id !== paymentMethodToDelete.id),
		);
		toast({
		...
		<div className="space-y-8">
			<div>
				<h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center">
					<Package className="mr-3 h-8 w-8 text-primary" /> Manage Your
					Subscription
				</h1>
				<p className="text-muted-foreground">
					View your current plan, manage payment methods, and make changes to
					your subscription.
				</p>
			</div>
					...
					<AlertDialog>
						...
						<AlertDialogContent>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									This action will cancel your Pro Plan subscription at the end
									of the current billing period (September 1, 2024). You will
									retain access to Pro features until then. This action cannot
									be undone through this interface.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
			...
			<Card className="shadow-md">
				...
				<CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
					{availablePlans.map((plan) => (
						<Card
							key={plan.id}
							className={`flex flex-col ${plan.isCurrent ? "border-primary ring-2 ring-primary" : "hover:shadow-lg"}`}
						>
							<CardHeader>
								<div className="flex items-center gap-3 mb-2">
									<plan.icon
										className={`h-7 w-7 ${plan.isCurrent ? "text-primary" : "text-muted-foreground"}`}
									/>
							...
							<CardContent className="flex-grow space-y-3">
								<ul className="space-y-2 text-sm">
									{plan.features.map((feature, index) => (
										<li
											key={index}
											className={`flex items-center ${feature.available ? "text-foreground" : "text-muted-foreground line-through"}`}
										>
											<CheckCircle
												className={`mr-2 h-4 w-4 ${feature.available ? "text-green-500" : "text-muted-foreground"}`}
											/>
											{feature.text}
										</li>
									))}
			...
			<Card className="shadow-md">
				<CardHeader>
					...
					<CardDescription>
						Manage your billing payment details. Add a new card or manage
						existing ones.
					</CardDescription>
				</CardHeader>
											...
											<AlertDialogTrigger asChild>
												<Button
													...
													disabled={
														...
														paymentMethodToDelete?.id === method.id
													}
												>
													{isDeletingPaymentMethod &&
													paymentMethodToDelete?.id === method.id ? (
													...
													<AlertDialogContent>
														<AlertDialogHeader>
															...
															<AlertDialogDescription>
																...
																{paymentMethodToDelete.cardType} ending in{" "}
																{paymentMethodToDelete.last4}? This action
																cannot be undone.
															</AlertDialogDescription>
														</AlertDialogHeader>
														<AlertDialogFooter>
