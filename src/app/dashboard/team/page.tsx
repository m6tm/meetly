...
export default function TeamPage() {
	...
	const [statusFilter, setStatusFilter] = React.useState<
		"all" | TeamMember["status"]
	>("all");
	const { toast } = useToast();

	const [isMemberFormDialogOpen, setIsMemberFormDialogOpen] =
	...
	const handleSaveMember = async () => {
		if (!memberEmail || !memberRole) {
			toast({
				...
				description:
					"Veuillez entrer une adresse e-mail et sélectionner un rôle.",
				variant: "destructive",
			});
			return;
		}
		...
		if (currentEditingMember) {
			const response = await updateTeamMemberRole(
				currentEditingMember.userId,
				memberRole,
			);
			if (response.success) {
				toast({
					title: "Membre mis à jour",
					description: `${currentEditingMember.name} a été mis à jour.`,
				});
				if (response.data) {
					setTeamMembers((prev) => [
						response.data!,
						...prev.filter((m) => m.email !== memberEmail),
					]);
				}
				loadTeamMembers();
			} else {
			...
			if (response.success) {
				toast({
					...
					description: `L'utilisateur ${memberEmail} a été traité.`,
				});
				if (response.data) {
					setTeamMembers((prev) => [
						response.data!,
	...
	const handleRemoveMember = async (member: TeamMember) => {
		...
		if (response.success) {
			toast({
				...
				description: `${member.name} a été supprimé.`,
				variant: "destructive",
			});
				...
				cell: ({ row }) => {
					...
					return (
						<Badge
							...
							className={
								status === TeamMemberStatus.ACTIVE
									? "bg-green-500/20 text-green-700 border-green-500/30 dark:bg-green-700/30 dark:text-green-300"
									: status === TeamMemberStatus.INVITED
										? "bg-blue-500/20 text-blue-700 border-blue-500/30 dark:bg-blue-700/30 dark:text-blue-300"
	...
	const dialogTitle = currentEditingMember
		? `Modifier: ${currentEditingMember.name}`
		: "Inviter un nouveau membre";
	const dialogDescription = currentEditingMember
		? "Mettez à jour les détails du membre ci-dessous."
		: "Entrez l'adresse e-mail.";
	const dialogButtonText = currentEditingMember
		? "Enregistrer les modifications"
		: "Envoyer l'invitation";
	const dialogIcon = currentEditingMember ? (
	...
	return (
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-2">
				...
				<Dialog
					open={isMemberFormDialogOpen}
					onOpenChange={handleOpenMemberFormDialog}
				>
					<DialogTrigger asChild>
						<Button
							onClick={() => setCurrentEditingMember(null)}
					...
					<DialogContent className="sm:max-w-md">
						...
						<div className="grid gap-4 py-4">
							...
							<div className="grid grid-cols-4 items-center gap-4">
								...
								<Select
									value={memberRole}
									onValueChange={(value) =>
										setMemberRole(value as TeamMember["role"])
									}
								...
								>
									...
									<SelectContent>
										...
										<SelectItem value={TeamMemberRole.EDITOR}>
											Éditeur
										</SelectItem>
						...
						<DialogFooter>
							<Button
								type="button"
								variant="outline"
								onClick={() => setIsMemberFormDialogOpen(false)}
								disabled={isSavingMember}
							>
								Annuler
							</Button>
							<Button
								...
								disabled={isSavingMember || !memberEmail || !memberRole}
							>
								{isSavingMember ? (
				...
				<CardContent>
					<div className="flex flex-col sm:flex-row items-center gap-2 py-4">
						...
						<Select
							value={roleFilter}
							onValueChange={(value) =>
								setRoleFilter(value as "all" | TeamMember["role"])
							}
						>
						...
						<Select
							value={statusFilter}
							onValueChange={(value) =>
								setStatusFilter(value as "all" | TeamMember["status"])
							}
						>
							<SelectTrigger className="w-full sm:w-[180px]">
