...
export default function RecordingsPage() {
	...
	const [statusFilter, setStatusFilter] = React.useState<
		"all" | RecordedMeeting["recordingStatus"]
	>("all");
	...
	const [recordingToDelete, setRecordingToDelete] =
		React.useState<RecordedMeeting | null>(null);
	const [isPermanentDeleteChecked, setIsPermanentDeleteChecked] =
		React.useState(false);
	...
	const handlePlayAudio = async (recording: RecordedMeeting) => {
		...
		try {
			const { success, data, error } = !recording.audioUrl
				? await getRecordingUrlAction(recording.filepath)
				: {
						success: true,
			...
			if (!success || !data) {
				toast({
					title: "Getting media url failled",
					description: error,
					variant: "destructive",
				});
				return;
			...
			if (data) {
				...
				toast({
					...
					description: `No audio URL found for recording: ${recording.title}`,
					variant: "destructive",
				});
			}
		} finally {
	...
	const handleStartTranscription = async (
		...
		setRecordings((prev) =>
			prev.map((rec) =>
				rec.id === recordingId
					? { ...rec, recordingStatus: "Transcription Pending" }
					: rec,
			),
		);
		toast({
			title: "Transcription Started",
			description: `Transcription process initiated for recording: ${recordingTitle}`,
		});
	};
	...
	const confirmDeleteRecording = async () => {
		...
		if (isPermanentDeleteChecked) {
			setRecordings((prev) =>
				prev.filter((rec) => rec.id !== recordingToDelete.id),
			);
			toast({
				title: "Recording Permanently Deleted",
				...
				prev.map((rec) =>
					rec.id === recordingToDelete.id
						? {
								...rec,
								recordingStatus: "Pending Deletion",
								previousStatus: rec.recordingStatus,
							}
						: rec,
				),
			);
			toast({
				title: "Recording Marked for Deletion",
				description: `"${recordingToDelete.title}" will be permanently removed in 30 days. You can restore it.`,
			});
		}
	...
	const handleUndoDelete = async (recordingId: string) => {
		...
		toast({
			...
			description: `The recording has been restored.`,
		});
	};
			...
			},
			{
				...
				cell: ({ row }) => {
					const status = row.getValue(
						"recordingStatus",
					) as RecordedMeeting["recordingStatus"];
					let icon = null;
					let badgeClasses = "";
					...
					switch (status) {
						...
						case "Pending Deletion":
							...
							badgeClasses =
								"bg-orange-500/20 text-orange-700 border-orange-500/30 hover:bg-orange-500/30 dark:bg-orange-700/30 dark:text-orange-300 dark:border-orange-700/40";
							break;
						default:
						...
						<div className="text-right whitespace-nowrap">
							{recording.filepath && (
								<Button
									...
									disabled={
										(!recording.filepath &&
											recording.recordingStatus !== "Transcription Pending") ||
										generatingMedialink === recording.id
									}
								>
									{generatingMedialink === recording.id ? (
							...
							<DropdownMenu>
								...
								<DropdownMenuContent align="end">
									{(recording.recordingStatus === "Recorded" ||
										...
										<DropdownMenuItem
											onClick={() =>
												handleStartTranscription(recording.id, recording.title)
											}
										>
									...
									{recording.recordingStatus === "Transcribed" && (
										<DropdownMenuItem
											onClick={() =>
												router.push(
													`/dashboard/transcriptions/?rec=${recording.id}`,
												)
											}
										>
				...
				<CardContent>
					<div className="flex flex-col sm:flex-row items-center gap-2 py-4">
						...
						<Select
							...
							onValueChange={(value) =>
								setStatusFilter(
									value as "all" | RecordedMeeting["recordingStatus"],
								)
							}
						>
							...
							<SelectContent>
								...
								<SelectItem value="Transcription Pending">
									Transcription Pending
								</SelectItem>
								<SelectItem value="Transcribed">Transcribed</SelectItem>
								<SelectItem value="Transcription Failed">
									Transcription Failed
								</SelectItem>
			...
			<Dialog open={isPlayerModalOpen} onOpenChange={handleOpenPlayerModal}>
				<DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl p-0">
						...
						{currentAudioUrl && (
							<>
									...
									<TooltipProvider delayDuration={0}>
										...
										>
											...
											<TooltipContent
												side="top"
												align="center"
												className="bg-black/80 text-white py-1 px-2 rounded text-xs"
											>
												<p>
													{tooltipTime !== null
								...
								<div className="mt-3 p-3 bg-background/70 rounded-md shadow flex items-center justify-between space-x-2">
									...
									{/* Spacer to balance play/pause button */}
								</div>
							</>
						)}
					</div>
					<DialogFooter className="p-4 border-t">
						<Button
							variant="outline"
							onClick={() => handleOpenPlayerModal(false)}
						>
							Close
						</Button>
					</DialogFooter>
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								<br />
								<br />
								If not deleted permanently, this recording will be marked for
								deletion and removed in 30 days. You can restore it during this
								period.
								<br />
								If you choose to delete permanently, this action is
								irreversible.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<div className="flex items-center space-x-2 my-4">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									setIsPermanentDeleteChecked(checked as boolean)
								}
							/>
