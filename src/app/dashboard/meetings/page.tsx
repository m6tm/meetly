...
export default function MeetingsPage() {
	const [meetings, setMeetings] =
		React.useState<Meeting[]>(initialMeetingsData);
	const [titleFilter, setTitleFilter] = React.useState("");
	const [statusFilter, setStatusFilter] = React.useState<
		"all" | Meeting["status"]
	>("all");
	const [typeFilter, setTypeFilter] = React.useState<"all" | Meeting["kind"]>(
		"SCHEDULE",
	);
	const [loading, setLoading] = React.useState(true);

	// State for the Schedule/Edit Meeting Dialog
	const [currentEditingMeeting, setCurrentEditingMeeting] =
		React.useState<Meeting | null>(null);
	...
	const handleFetchMeetings = useCallback(async () => {
		...
		if (!response.success) {
			toast({
				...
				action: (
					<Button variant="outline" onClick={handleFetchMeetings}>
						Réessayer
					</Button>
				),
			});
	...
	const handleCancelMeeting = async (meetingId: string) => {
		...
		if (response.success) {
			...
			if (fetchResponse.success && fetchResponse.data) {
				const meetFormated: Meeting[] = fetchResponse.data.meetings.map(
					(meeting) => {
						const now = new Date();
						let status: Meeting["status"];
						if (meeting.cancelled) {
	...
	const handleTranscribeMeeting = (meetingId: string) => {
		...
		toast({
			...
			description: `Transcription process initiated for meeting ${meetingId}`,
		});
	};
		...
		<div className="space-y-6">
					...
					<ScheduleMeetingModal
						{...{
							initialValues: {
								...
								invitees: currentEditingMeeting.original_meet.invitees.map(
									(invite) => invite.email,
								),
			...
			<Card className="shadow-md overflow-x-auto">
				<CardHeader>
					...
					<CardDescription>
						Overview of all your scheduled and past meetings. Apply filters to
				...
				<CardContent>
					<div className="flex flex-col sm:flex-row items-center gap-2 py-4">
						...
						<Select
							value={statusFilter}
							onValueChange={(value) =>
								setStatusFilter(value as "all" | Meeting["status"])
							}
						>
						...
						<Select
							value={typeFilter}
							onValueChange={(value) =>
								setTypeFilter(value as "all" | Meeting["kind"])
							}
						>
							<SelectTrigger className="w-full sm:w-[180px]">
