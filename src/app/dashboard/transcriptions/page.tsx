...
export default function TranscriptionsPage() {
	const [transcribedMeetings, setTranscribedMeetings] = React.useState<
		TranscribedMeeting[]
	>([]);
	const [titleFilter, setTitleFilter] = React.useState("");
	const [statusFilter, setStatusFilter] = React.useState<
		"all" | TranscribedMeeting["transcriptionStatus"]
	>("all");
	...
	const [selectedMeetingDetails, setSelectedMeetingDetails] =
		React.useState<TranscribedMeeting | null>(null);
	...
	const handleFetchRecordings = React.useCallback(async () => {
		...
		if (response.success && response.data) {
			const formattedRecordings = response.data.recordings.map((rec) => {
				let transcriptionStatus: TranscribedMeeting["transcriptionStatus"] =
					"Pending";
				...
				return {
					...
					duration: formatToHumanReadable(
						Number(rec.meetingRecordingPath!.duration),
					),
	...
	const handleRetryTranscription = async (
		...
		toast({
			...
			description: `Transcription process re-initiated for meeting ${meetingTitle}`,
		});
	...
	const handleTranscription = async (
		...
		toast({
			title: "Transcribing",
			description: `Transcription process started for meeting ${meetingTitle}`,
		});
		const response = await startTranscriptionAction(meetingId);
		if (!response.success) {
			toast({
	...
	const PDFDocument = ({
		...
		meeting: typeof selectedMeetingDetails;
		...
		<div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
			...
			<div style={{ marginBottom: "20px" }}>
				<p>
					<strong>Date:</strong>{" "}
					{meeting?.date ? new Date(meeting.date).toLocaleString() : "N/A"}
				</p>
				<p>
					<strong>Status:</strong> {meeting?.transcriptionStatus}
				</p>
			</div>
		...
		try {
			...
			const html = await compile(
				<PDFDocument
					...
					transcription={
						selectedMeetingDetails.fullTranscription ??
						"Aucune transcription disponible"
					}
					summary={selectedMeetingDetails.summary ?? "Aucun résumé disponible"}
				/>,
			);
			...
			const opt = {
				margin: 10,
				filename: `${selectedMeetingDetails.title
					.replace(/[^a-z0-9]/gi, "_")
	...
	const handleExportMarkdown = async () => {
        ...
        **Date:** ${
					selectedMeetingDetails.date
						? new Date(selectedMeetingDetails.date).toLocaleString()
						: "N/A"
				}\
      ...
      `;
			...
			a.href = url;
			a.download = `${selectedMeetingDetails.title
				.replace(/[^a-z0-9]/gi, "_")
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Une erreur est survenue lors de la génération du fichier Markdown.",
				variant: "destructive",
			});
		}
	};
	...
	const handleSendEmail = () => {
		...
		toast({
			title: "Send by Email",
			description: `Preparing email for "${selectedMeetingDetails.title}" (simulated).`,
		});
	};
	...
	const columns: ColumnDef<TranscribedMeeting>[] = React.useMemo(
			...
			},
			{
				...
				cell: ({ row }) => {
					const status = row.getValue(
						"transcriptionStatus",
					) as TranscribedMeeting["transcriptionStatus"];
					...
					switch (status) {
						case "Completed":
							variant = "default";
							badgeClasses =
								"bg-green-500/20 text-green-700 border-green-500/30 hover:bg-green-500/30 dark:bg-green-700/30 dark:text-green-300 dark:border-green-700/40";
							icon = <FileText className="mr-1.5 h-3.5 w-3.5" />;
							break;
						case "Pending":
							variant = "secondary";
							badgeClasses =
								"bg-yellow-500/20 text-yellow-700 border-yellow-500/30 hover:bg-yellow-500/30 dark:bg-yellow-700/30 dark:text-yellow-300 dark:border-yellow-700/40";
						...
						case "Processing":
							variant = "outline";
							badgeClasses =
								"bg-blue-500/20 text-blue-700 border-blue-500/30 hover:bg-blue-500/30 dark:bg-blue-700/30 dark:text-blue-300 dark:border-blue-700/40";
						...
						<div className="text-right whitespace-nowrap">
							...
							{meeting.transcriptionStatus === "Failed" && (
								<Button
									...
									size="sm"
									onClick={() =>
										handleRetryTranscription(meeting.id, meeting.title)
									}
								>
		...
		// biome-ignore lint/correctness/useExhaustiveDependencies: Pour supprimer l'avertissement sur les dépendances du hook
		[transcribedMeetings],
	);
	...
	const filteredData = React.useMemo(() => {
		return transcribedMeetings
			.filter((meeting) =>
				titleFilter
					? meeting.title.toLowerCase().includes(titleFilter.toLowerCase())
					: true,
			)
			.filter((meeting) =>
				statusFilter !== "all"
					? meeting.transcriptionStatus === statusFilter
					: true,
			);
				...
				<CardContent>
					<div className="flex flex-col sm:flex-row items-center gap-2 py-4">
						...
						<Select
							...
							onValueChange={(value) =>
								setStatusFilter(
									value as "all" | TranscribedMeeting["transcriptionStatus"],
								)
							}
						>
					...
					<DialogContent className="h-screen w-screen max-w-none sm:max-w-none rounded-none flex flex-col p-0">
						<DialogHeader className="p-4 border-b flex-shrink-0">
							...
							{selectedMeetingDetails.date && selectedMeetingDetails.time && (
								<DialogDescription>
									Meeting Date:{" "}
									{format(parseISO(selectedMeetingDetails.date), "PPP")}
									{" at "}
										...
										<ScrollArea className="h-[calc(100vh-280px)] lg:h-[calc(100vh-240px)]">
											...
											<div className="whitespace-pre-wrap text-sm leading-relaxed">
												<MarkdownViewer
													content={
														selectedMeetingDetails.fullTranscription ||
														"No full transcription available."
													}
												/>
											</div>
										</ScrollArea>
									...
									<CardContent className="space-y-4">
										{selectedMeetingDetails.summaryAvailable ? (
												...
												{selectedMeetingDetails.summary && (
													<div>
														...
														<div className="text-sm">
															<MarkdownViewer
																content={selectedMeetingDetails.summary}
															/>
														</div>
													</div>
												)}
											...
											<div className="text-sm text-muted-foreground">
												AI summary is not available for this meeting.
											</div>
										)}
									</CardContent>
								</Card>
...
const BrainIcon = (props: React.SVGProps<SVGSVGElement>) => (
	<svg
		...
		{...props}
	>
		<path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v1.13a1 1 0 0 0 .8.98C15.62 7.11 17.5 9.63 17.5 12.5c0 3.15-2.5 5.5-5.5 5.5S6.5 15.65 6.5 12.5c0-2.88 1.88-5.39 4.7-5.89a1 1 0 0 0 .8-.98V4.5A2.5 2.5 0 0 1 12 2a2.5 2.5 0 0 1 2.5-2.5M12.5 18a3.5 3.5 0 0 1-3.5-3.5c0-.5.44-1.15.66-1.4a6.5 6.5 0 0 0 5.68 0c.22.25.66.9.66 1.4a3.5 3.5 0 0 1-3.5 3.5Z" />
		<path d="M4.5 12.5c0-3.15 2.5-5.5 5.5-5.5M14 18.5c-1.5 0-2.5-.5-2.5-1.5a2.49 2.49 0 0 1 .37-1.3" />
		<path d="M10 18.5c1.5 0 2.5-.5 2.5-1.5a2.49 2.49 0 0 0-.37-1.3" />
		<path d="M19.5 12.5c0-3.15-2.5-5.5-5.5-5.5" />
	</svg>
);
