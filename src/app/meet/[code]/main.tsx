...
import {
	generateMeetTokenAction,
	type MeetTokenDataType,
...
function Main({ userData, token, isAuthed, hasPassword }: MeetPageProps) {
	const [isInLobby, setIsInLobby] = useState(true);
	const [displayName, setDisplayName] = useState(
		userData.participantName === "undefined" ? "" : userData.participantName,
	);

	const [activeSidePanel, setActiveSidePanel] = useState<
		"chat" | "participants" | "info" | null
	>(null);
	...
	const [joinMeetingToastDetails, setJoinMeetingToastDetails] = useState<{
		title: string;
		description: string;
	} | null>(null);
	const [feedbackToastDetails, setFeedbackToastDetails] = useState<{
		title: string;
		description?: string;
	...
	const handleCopyMeetingLink = async () => {
		...
		try {
			...
			setFeedbackToastDetails({
				title: "Lien copié",
				description:
					"Le lien de la réunion a été copié dans le presse-papiers.",
			});
		} catch (err) {
	...
	const handleCheckUser = React.useCallback(async () => {
		...
		if (!response.success || !response.data) {
			toast({
				...
				description:
					"Assuez vous que vous avez une bonne connexion ou que vous avez été invité. Vous serez redirigé dans quelques secondes.",
			});
			setTimeout(() => {
