...
import type { TranscriptionStartData } from "@/types/meetly.types";
...
const transcriptionStart = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		if (
			...
			(!downloadFileResponse.success &&
				!downloadFileResponse.file &&
				!downloadFileResponse.contentType)
		) {
		...
		const aiModel = await step.run(
			"transcription.processing.get-model",
			async () => {
				try {
				...
				contents: [
					...
					},
					{
						...
						parts: [
							{
								inlineData: {
									data: downloadFileResponse.file!.replace(
										/^data:audio\/\w+;base64,/,
										"",
									),
