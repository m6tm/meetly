...
import type { SummaryStartData } from "@/types/meetly.types";
...
const summaryStart = inngest.createFunction(
	...
	async ({ event, step }) => {
		const { recordingId, transcription, retry_count } =
			event.data as SummaryStartData;
						...
						safetySettings: [
							{
								...
								category:
									GeminiAiAdapter.HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT,
							},
						],
						generationConfig: {
