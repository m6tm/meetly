...
const startRecording = inngest.createFunction(
	...
	async ({ event, step }) => {
		const { egressId, meetingId, meet_name, filepath, retry_count } =
			event.data as RecordingStartData;
		...
		const meetingRecordingUpdated = await step.run(
			"creating-new-meeting-recording",
			async () => {
				try {
...
const stopRecording = inngest.createFunction(
	...
	async ({ event, step }) => {
		const { egressId, meetingId, duration, retry_count } =
			event.data as StopRecordingPayload;
		let _error: Error | null = null;
		const prisma = getPrisma();
		...
		const meetingRecording = await step.run(
