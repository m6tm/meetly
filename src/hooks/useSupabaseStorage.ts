...
import type { UploadConfig, UploadProgress } from "@/types/supabase.types";
import {
	SupabaseUploader,
	type UploadResult,
} from "@/utils/supabase/supabase_s3_upload";
...
export const useSupabaseStorage = () => {
	...
	const uploadFile = useCallback(
		async (file: File, config?: UploadConfig) => {
			setIsUploading(true);
			setError(null);
	...
	const uploadLargeFile = useCallback(
		async (file: File, config?: UploadConfig) => {
			...
			try {
				...
				const result = await uploader.uploadLargeFileWithS3(
					file,
					config,
					(progress) => {
	...
	const listFiles = useCallback(
		async (bucketName: string, folder?: string) => {
			setError(null);
